...
export default async function LeaderboardPage({
  ...
  const signedUrls = await signScreenshots([
    ...current.entries.map((e) => e.screenshotPath),
    ...previous.entries.map((e) => e.screenshotPath),
  ]);
  ...
  return (
    <Shell>
      ...
      {windowOpen && currentWeek && (
        <div
          className="fixed bottom-[62px] inset-x-0 px-4 pb-3"
        ...
        >
          <div className="mx-auto max-w-[560px]">
            ...
            >
              {mine
                ? `Change your ${formatMinutes(mine.minutes)}`
                : "Upload screenshot"}
            </Link>
